...
# from flask import Flask, request, jsonify, Response, send_from_directory, abort, current_app
...
# TUTOR_SYSTEM_TEXT = (
#     "Respond in the Format is understandable by eleven_multilingual_v2 model"
#     "You are a world-class tutor with PhD-level knowledge. "
#     "You teach intuitively and clearly. Start with a concise answer, "
#     "then explain step-by-step in simple language. Use examples or analogies "
#     "when helpful. Prefer bullets and short paragraphs. "
# )

# # Prompt for RetrievalQA (Ask Once). The 'stuff' chain expects {context} and {question}.
...
#         "Context:\n{context}\n\n"
#         "Question: {question}\n\n"
#         "Answer as a patient, intuitive tutor. Keep it clear, step-by-step, "
#         "and include an example if useful."
#     ),
# ).partial(system=TUTOR_SYSTEM_TEXT)

# # ------------ Chat Chains ------------
# CHAINS: Dict[str, LLMChain] = {}

# def build_chain() -> LLMChain:
#     """Prompt + Memory + Chain with tutoring persona."""
#     prompt = ChatPromptTemplate.from_messages([
#         SystemMessagePromptTemplate.from_template(TUTOR_SYSTEM_TEXT),
...
# def get_chat_chain(session_id: str) -> LLMChain:
#     if session_id not in CHAINS:
#         CHAINS[session_id] = build_chain()
...
#     return jsonify({"detail": f"Stored {len(chunks)} chunks in ChromaDB"})
...
#     # RetrievalQA with tutoring persona prompt
...
# # /chat — Stateful conversation (session + memory + retrieval)
# @app.post("/chat")
# def chat():
#     payload = request.get_json(silent=True) or {}
...
#     out = chain.invoke({
#         "text": (
#             "Use this (optional) context if relevant:\n"
#             f"{context}\n\n"
#             f"Now answer the user's message: {message}"
#         )
#     })
#     answer = out.get("text", "")
...
# main.py (fixed history factory + modern LangChain + in-memory Chroma)
import os
import uuid
import traceback
